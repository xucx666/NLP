...
class MyBertModel(BertPreTrainedModel):
    ...
    def forward_mix_embed(self, x1, token_type1, att1, x2, token_type2, att2, lam):
        ...
        encoder_attention_mask, extended_attention_mask1, encoder_extended_attention_mask = self._forward_init(
            input_ids=x1, attention_mask=att1, token_type_ids=token_type1)
        ...
        encoder_attention_mask, extended_attention_mask2, encoder_extended_attention_mask = self._forward_init(
            input_ids=x2, attention_mask=att2, token_type_ids=token_type2)
...
class TextBERT(nn.Module):
    def __init__(self, args):
        ...
        self.config = os.path.join(args.bert_dir, 'config.json')
        self.output_dim = args.class_num
        self.bert = MyBertModel.from_pretrained(self.config)
        # Freeze bert layers
        if not args.fine_tune:
            for p in self.bert.parameters():
                p.requires_grad = False

        bert_dim = args.feature_num
